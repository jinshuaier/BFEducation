...
//        [self addSubview:loadActivity];
        ...
        _middleBtn.frame = CGRectMake((KScreenW - 50)/2, (KScreenH - 50)/2, 50, 50);
        [_middleBtn setImage:[UIImage imageNamed:@"big_b"] forState:UIControlStateNormal];
        [_middleBtn setImage:[UIImage imageNamed:@"bofang_dianji"] forState:UIControlStateSelected];
        [_middleBtn addTarget:self action:@selector(clickChangeVideoStatus) forControlEvents:UIControlEventTouchUpInside];
...
-(void)clickChangeVideoStatus {
    if (_middleBtn.selected) {
        NSLog(@"点击播放");
        [_player play];
        [self.playBtn setImage:[UIImage imageNamed:@"MoviePlayer_Play"] forState:UIControlStateNormal];
        [self.middleBtn setImage:[UIImage imageNamed:@"big_b"] forState:UIControlStateNormal];
    }
    else {
        NSLog(@"点击暂停");
        [_player pause];
        [self.playBtn setImage:[UIImage imageNamed:@"MoviePlayer_Stop"] forState:UIControlStateNormal];
        [self.middleBtn setImage:[UIImage imageNamed:@"bofang_dianji"] forState:UIControlStateNormal];
    }
    _middleBtn.selected =!_middleBtn.selected;
}
// 后台
...
- (void)setlandscapeLayout{
    ...
    [self.totateBtn setImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
}
...
- (IBAction)playORPause:(id)sender {
    if (_isPlaying) {
        [self pause];
        [_middleBtn setImage:[UIImage imageNamed:@"bofang_dianji"] forState:UIControlStateNormal];
    }else{
        [self play];
        [_middleBtn setImage:[UIImage imageNamed:@"big_b"] forState:UIControlStateNormal];
    }
...
- (void)portraitShow{
    ...
    //设置底部视图渐变色
    UIColor *color = [UIColor colorWithPatternImage:[UIImage imageNamed:@"mengban"]];
