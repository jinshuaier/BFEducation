...
#import "BFPlayBackViewController.h"
...
@interface BFPlayBackViewController ()<UITextFieldDelegate,RequestDataPlayBackDelegate,UITableViewDelegate,UITableViewDataSource>
...
static NSString *playLiveCellId = @"playLiveCellId";
@implementation BFPlayBackViewController
...
- (void)viewDidLoad {
    ...
    [_tableView registerClass:[BFPlayLiveCell class] forCellReuseIdentifier:playLiveCellId];
    _tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(getDataSource)];
...
- (void)getDataSource{
    NSString *url = [GetFromUserDefaults(@"stuOrTeac") isEqualToString:@"stu"] ? [NSString stringWithFormat:@"%@?stuId=%@",FindBackRoomURL_STU,GetFromUserDefaults(@"user_id")] : FindBackRoomURL_TEAC;
    [NetworkRequest requestWithUrl:url parameters:nil successResponse:^(id data) {
        NSArray *arr = data;
        [_playBackArray removeAllObjects];
        for (NSDictionary *dic in arr) {
            BFPlayLive *playLive = [BFPlayLive new];
            [playLive setValuesForKeysWithDictionary:dic];
            [_playBackArray addObject:playLive];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    BFPlayLiveCell *cell = [tableView dequeueReusableCellWithIdentifier:playLiveCellId];
    if (!cell) {
        cell = [[BFPlayLiveCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:playLiveCellId];
    }
    cell.playLive = self.playBackArray[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    BFPlayLive *playLive = _playBackArray[indexPath.row];
    if ([GetFromUserDefaults(@"stuOrTeac") isEqualToString:@"stu"] && [playLive.order isEqualToString:@"0"]) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"您还没有观看这个直播间直播的权限" preferredStyle:(UIAlertControllerStyleAlert)];
    ...
    PlayBackViewController *playBackVC = [PlayBackViewController new];
    playBackVC.playLive = playLive;
