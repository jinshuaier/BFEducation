...
@interface BFWatchCourseVC ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,DWGestureViewDelegate,UIAlertViewDelegate,DWVideoPlayerDelegate,TreeTableCellDelegate>
{
...
@implementation BFWatchCourseVC{
    UILabel *titleLabel;
    UILabel *lineLabel;
}
...
//    [MobClick beginLogPageView:@"观看课程页面"];
}
...
//    [MobClick endLogPageView:@"观看课程页面"];
}
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        ...
        [self backButtonAction:_backButton];
...
- (void)startRequestAdInfo
{
    ...
    if ([_type isEqualToString:@"2"]) {
        //暂停广告
        __weak BFWatchCourseVC *blockslf = self;
    ...
    if ([_type isEqualToString:@"1"]) {
        //片头广告
        __weak BFWatchCourseVC *blockself = self;
        _adInfo.finishBlock = ^(NSDictionary *response){
...
//    [self.backButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self.backButton setImage:[UIImage imageNamed:@"返回-白"] forState:UIControlStateNormal];
...
//    [self.overlayView addSubview:self.vrView];
    ...
    if (KIsiPhoneX) {
        _overlayView.frame = CGRectMake(0, 44, frame.size.width, PlayViewHeight);
        _videoBackgroundView.frame = _overlayView.frame;
    }
...
- (void)setupUI{
    if (_isInstroduce) {
        [self getIntroduce];
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 150, KScreenW, KScreenH - 64 - 150) style:(UITableViewStylePlain)];
        ...
        _treeTableView = [[TreeTableView alloc] initWithFrame:CGRectMake(0, 150, KScreenW, KScreenH - 64 - 150) withDict:_catalogueDic];
...
- (void)layout{
    if (_isInstroduce) {
        ...
        .leftEqualToView(self.view)
        .topSpaceToView(self.overlayView, 0)
        .rightEqualToView(self.view)
        .heightIs(50);
...
- (void)setCatalogueDic:(NSMutableDictionary *)catalogueDic{
    _catalogueDic = catalogueDic;
}
...
// 获取目录
- (void)getCatalogue{
    NSString *url = [NSString stringWithFormat:@"%@?cSId=%ld",CourseDirectoryURL,_cid];
...
- (void)getIntroduce{
    ...
    if ([str isEqualToString:@"1"]) { //用户已登录
        url = [NSString stringWithFormat:@"%@?ckey=%d&ciid=%ld&uid=%@",CourseCatalogueURL,1,_cid,GetFromUserDefaults(@"uId")];
    }else{
        url = [NSString stringWithFormat:@"%@?ckey=%d&ciid=%ld&uid=%@",CourseCatalogueURL,1,_cid,@"0"];
    }
    
    [NetworkRequest requestWithUrl:url parameters:nil successResponse:^(id data) {
        NSDictionary *dic = data;
//        _descriptionStr = [self flattenHTML:[NSString stringWithFormat:@"%@",dic[@"mapssMap"][@"cidesc"]]];
        _descriptionStr = dic[@"mapssMap"][@"cidesc"];
        dispatch_async(dispatch_get_main_queue(), ^{
