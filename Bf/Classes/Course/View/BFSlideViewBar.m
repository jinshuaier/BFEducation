...
@interface BFSlideViewBar () <UIScrollViewDelegate> {
...
@implementation BFSlideViewBar

#pragma mark - setter (Override)
- (void)setDelegate:(id<BFSlideViewBarDelegate>)delegate {
...
- (void)addSlideBar {
    ...
    if (!_slideBar) {
        ...
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, SlideBar_Height - 0.5, KScreenW, 0.5)];
        lineView.backgroundColor = [UIColor lightGrayColor];
        [_slideBar addSubview:lineView];
...
//    self.childControllersArray = [self.delegate childViewControllersInSlideView:self];
    for (UIViewController *vc in self.childControllersArray) {
        ...
        if (tableView) {
            tableView.frame = CGRectMake(0, 0, vc.view.width, vc.view.height);
        }
...
- (void)p_animateSliderWithTag:(NSInteger)tag {
    [self p_animateSliderToPositionWithOffset:CGPointMake(tag, 0)];
...
- (void)p_animateSliderToPositionWithOffset:(CGPoint)offset {
    ...
    CGFloat sliderWidth = slideItemWidth * SliderThanSliderView_WidthRatio;
    CGFloat position_x = (slideItemWidth - sliderWidth)/2.0;
    CGRect newFrame = CGRectMake((offset.x)*slideItemWidth+position_x, _slider.frame.origin.y, _slider.frame.size.width, _slider.frame.size.height);
...
- (void)setCurrentIndex:(NSInteger)currentIndex{
    CGPoint point = CGPointMake(currentIndex, 0);
    [self p_animateSliderToPositionWithOffset:point];
}
