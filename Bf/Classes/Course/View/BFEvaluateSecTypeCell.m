...
@interface BFEvaluateSecTypeCell ()
// 头像
...
@implementation BFEvaluateSecTypeCell
...
- (void)setupUI{
    ...
    _likeBtn.titleLabel.font = [UIFont fontWithName:BFfont size:12];
    [_likeBtn setTitleColor:RGBColor(153, 153, 153) forState:(UIControlStateNormal)];
    _likeBtn.imageEdgeInsets = UIEdgeInsetsMake(0, -6, 0, 0);
...
- (void)setModel:(BFEvaluateModel *)model{
    ...
    _evaluateLabel.text = _model.pCComment;
    [_likeBtn setImage:_model.comFlag ? [UIImage imageNamed:@"赞-3拷贝2"] : [UIImage imageNamed:@"赞-3拷贝6"] forState:(UIControlStateNormal)];
    [_likeBtn setTitle:[NSString stringWithFormat:@"%ld",_model.postComCount] forState:(UIControlStateNormal)];
}
