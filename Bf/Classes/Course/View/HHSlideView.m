...
#define HighlightedTitle_DefaultColor   [UIColor blueColor]
...
//    [button setTitleColor:[_slider backgroundColor] forState:UIControlStateNormal];
    ...
    [self p_animateSliderToPositionWithOffset:CGPointMake(button.tag * KScreenW, 0)];
...
- (void)setCurrentIndex:(NSInteger)currentIndex{
    ...
    CGFloat sliderWidth = slideItemWidth * SliderThanSliderView_WidthRatio;
    CGFloat position_x = (slideItemWidth - sliderWidth)/2.0;
    CGRect newFrame = CGRectMake((currentIndex/SCREEN_WIDTH)*slideItemWidth+position_x, _slider.frame.origin.y, _slider.frame.size.width, _slider.frame.size.height);
    [_slider setFrame:newFrame];
}
