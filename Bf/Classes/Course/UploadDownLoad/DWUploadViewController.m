...
@property(nonatomic,strong)InformationShowView  *informationView;// 提示框
@end
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if ([data length] /1024.0/1024.0 > MaxUplodeFileSize) {
        _informationView = [[InformationShowView alloc] initWithLabel:@"该视频文件大于2.0M"];
...
//    NSError *error;
//    if ([DWTools getFileSizeWithPath:self.videoPath Error:&error]/1024.0/1024.0 > MaxUplodeFileSize) {// 上传视频大于2兆提示
//        _informationView = [[InformationShowView alloc] initWithLabel:@"该视频文件大于2.0M"];
//        [self.view addSubview:_informationView];
//        [_informationView mas_makeConstraints:^(MASConstraintMaker *make) {
//            make.edges.mas_equalTo(UIEdgeInsetsMake(0, 0, 0, 0));
...
- (void)setUploadBlockWithItem:(DWUploadItem *)item andCell:(DWUploadTableViewCell *)cell
{
    ...
    uploader.videoContextForRetryBlock = ^(NSDictionary *videoContext) {
        ...
        NSString *url = [[NSString stringWithFormat:@"%@?videoId=%@&videoName=%@&videoSize=%@&videoDate=%@&teacId=%@",AddVideoURL,videoContext[@"videoid"],videoContext[@"filename"],@([DWTools getFileSizeWithPath:videoContext[@"filepath"] Error:nil]),videoContext[@"createtime"],GetFromUserDefaults(@"user_id")] stringByAddingPercentEscapesUsingEncoding:(NSUTF8StringEncoding)];
