 ...
 *  @param attributedString 需要被导出的富文本
 *
 *  @param array            需要被导出的图片地址
 *
 *  @param textStr          需要被导出的纯文本
 *
 *  @return 导出的 HTML
 */
+ (NSString *)HTMLFromAttributedString:(NSAttributedString *)attributedString withImagePathArray:(NSMutableArray **)array withTextString:(NSMutableString **)textStr{
    ...
    while (effectiveRange.location + effectiveRange.length < attributedString.length) {
        ...
        if (attachment) {
            switch (attachment.attachmentType){
                case LMTextAttachmentTypeImage:{
                    [htmlContent appendString:[NSString stringWithFormat:@"<img src=\"%d\" width=\"100%%\"/>", imageNum]];
                    NSURL *url = attachment.userInfo;
            ...
            NSString *text = [[attributedString string] substringWithRange:effectiveRange];
            NSString *newText = [NSString filterHtml:text];
            [str appendString:newText];
            UIFont *font = attributes[NSFontAttributeName];
            UIColor *fontColor = attributes[@"NSColor"];
            NSString *color = [self hexStringWithColor:fontColor];
            BOOL underline = [attributes[NSUnderlineStyleAttributeName] boolValue];
