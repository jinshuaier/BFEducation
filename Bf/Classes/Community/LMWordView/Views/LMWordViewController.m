...
@interface LMWordViewController () <UITextViewDelegate, UITextFieldDelegate, LMSegmentedControlDelegate, LMStyleSettingsControllerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
- (void)layoutTextView {
    ...
    if (imageORVideo == 2) {
        ...
        if (!isLayoutVideo) {
            rect.size.height -= (VideoViewHeight + 10);
            isLayoutVideo = YES;
        }
    }else{
        if (isLayoutVideo) {
...
- (UIImageView *)videoView{
    if (!_videoView) {
        _videoView = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.view.height - VideoViewHeight - 10, KScreenW - 40, VideoViewHeight)];
    }
    return _videoView;
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if ([[info objectForKey:UIImagePickerControllerMediaType] isEqualToString:@"public.image"]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self layoutTextView];
            self.videoView.image = self.image;
...
//    }
//    else if (self.textView.titleTextField.text.length < 5) {
