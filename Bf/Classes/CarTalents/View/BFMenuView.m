...
@interface BFMenuView()<UITableViewDelegate,UITableViewDataSource>
...
-(instancetype)initWithFrame:(CGRect)frame titleArr:(NSArray *)titleArr imgArr:(NSArray *)imgArr arrowOffset:(CGFloat)arrowOffset rowHeight:(CGFloat)rowHeight layoutType:(menuViewLayoutType)layoutType directionType:(menuViewDirectionType)directionType delegate:(id<menuViewDelegate>)delegate{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        for (int i = 0; i < titleArr.count; i++) {
            ...
            menuViewModel *model = [menuViewModel menuViewWithDictonary:tempDic];
...
-(void)drawRect:(CGRect)rect{
    ...
    if (_directionType == menuViewDirectionUp) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    menuViewCell *cell = [menuViewCell MenuViewCellWithTableView:tableView];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    menuViewModel *model = self.titleImgArr[indexPath.row];
...
+(instancetype)MenuViewCellWithTableView:(UITableView *)tableView{
    ...
    if (!cell) {
        cell = [[menuViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
    return cell;
}
#pragma mark - 重写
...
-(void)layoutSubviews{
    ...
    if (self.layoutType == menuViewLayoutTypeTitle) {
...
-(void)setMenuViewModel:(menuViewModel *)menuViewModel{
