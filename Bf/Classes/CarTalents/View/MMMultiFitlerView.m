...
- (void)popupViewFromSourceFrame:(CGRect)frame completion:(void (^ __nullable)(void))completion {
    ...
    //出现的动画
    LeftCellWidth  = (self.item.numberOflayers == MMPopupViewThreelayers)?kScreenWidth/2:kScreenWidth/2;
    [UIView animateWithDuration:AnimationDuration animations:^{
        ...
        self.bottomView.frame = CGRectMake(0, self.tableView.bottom, self.width, PopupViewTabBarHeight);
        [self addSubview:self.bottomView];
        completion();
        //添加底部按钮
        UIButton *btn0 = [UIButton buttonWithType:UIButtonTypeCustom];
        btn0.frame = CGRectMake(0, 0, KScreenW/2, 40);
        btn0.tag = 0;
        [btn0 setTitle:@"重置" forState:UIControlStateNormal];
        [btn0 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        btn0.titleLabel.font = [UIFont fontWithName:BFfont size:14.0f];
        btn0.backgroundColor = [UIColor whiteColor];
        [btn0 addTarget:self action:@selector(clickBtn0Action) forControlEvents:UIControlEventTouchUpInside];
        [self.bottomView addSubview:btn0];
        
        UIButton *btn1 = [UIButton buttonWithType:UIButtonTypeCustom];
        btn1.frame = CGRectMake(KScreenW/2, 0, KScreenW/2, 40);
        btn1.tag = 1;
        [btn1 setTitle:@"确定" forState:UIControlStateNormal];
        [btn1 setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        btn1.titleLabel.font = [UIFont fontWithName:BFfont size:14.0f];
        btn1.backgroundColor = RGBColor(0, 148, 231);
        [btn1 addTarget:self action:@selector(clickBtn1Action) forControlEvents:UIControlEventTouchUpInside];
        [self.bottomView addSubview:btn1];
    }];
    
}

-(void)clickBtn0Action {
    NSLog(@"点击重置 ++++++++");
    [self dismiss];
...
-(void)clickBtn1Action {
    NSLog(@"点击确定 ++++++++");
    [self dismiss];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"sureLocation" object:nil];
}
